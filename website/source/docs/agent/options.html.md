...
* <a name="_enable_script_checks"></a><a href="#_enable_script_checks">`-enable-script-checks`</a> This
  ...
  this agent, both from local config files and from API, and defaults to `false` so operators must opt-in
  to allowing these. If enabled, it is recommended to [enable ACLs](/docs/guides/acl.html) as well to
  control which users are allowed to register new checks to execute scripts. This was added in Consul 0.9.0.

* <a name="_enable_local_script_checks"></a><a href="#_enable_local_script_checks">`-enable-local-script-checks`</a> This
  controls whether [health checks that execute scripts](/docs/agent/checks.html) defined in **local config files**
  are enabled. To allow for creating script checks from the HTTP API see <a href="#_enable_script_checks">`-enable-script-checks`</a>
  for both, or <a href="#_enable_remote_script_checks">`-enable-remote-script-checks`</a> for HTTP API only.

* <a name="_enable_remote_script_checks"></a><a href="#_enable_remote_script_checks">`-enable-remote-script-checks`</a> This
  controls whether [health checks that execute scripts](/docs/agent/checks.html) created via **HTTP API calls**
  are enabled. To allow for creating script checks from the local config files see <a href="#_enable_script_checks">`-enable-script-checks`</a>
  for both, or <a href="#_enable_local_script_checks">`-enable-local-script-checks`</a> for local config files only.
